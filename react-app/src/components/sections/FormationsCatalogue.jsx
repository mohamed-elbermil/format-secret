...
const formationMeta = {
  ...
  formation15: { duration: '1 jour',  modality: 'Présentiel', desc: 'Transformez vos réunions en moments productifs : organisation, animation et méthodes collaboratives.' },
  formation7:  { duration: '2 jours', modality: 'Présentiel', desc: 'Gagnez en aisance à l\'oral, apprivoisez le trac et rendez vos discours captivants face à tout type d\'audience.' },
  formation16: { duration: '1 jour',  modality: 'Présentiel', desc: 'Maîtrisez les règles d\'or de l\'accueil de qualité et les techniques de communication téléphonique professionnelle.' },
  formation14: { duration: '1 jour',  modality: 'Présentiel', desc: 'Rédigez des textes clairs et percutants : prise de notes, structuration, vocabulaire précis pour convaincre.' },
  formation5:  { duration: '2 jours', modality: 'Présentiel', desc: 'De l\'accueil client à la conclusion : maîtrisez les techniques de vente et boostez votre performance commerciale.' },
  formation6:  { duration: '1 jour',  modality: 'Présentiel', desc: 'Gérez le changement, maintenez votre bien-être et préparez sereinement cette nouvelle étape de vie.' },
  formation20: { duration: '1 jour',  modality: 'Présentiel', desc: 'Adoptez les bonnes postures et techniques ergonomiques pour préserver votre santé et prévenir les TMS.' },
  formation21: { duration: '2 jours', modality: 'Présentiel', desc: 'Devenez acteur de la sécurité : apprenez à intervenir, prodiguer les premiers secours et alerter efficacement.' },
  formation22: { duration: '1 jour',  modality: 'Présentiel', desc: 'Actualisez votre certification SST : révisez les gestes qui sauvent et renforcez votre rôle en prévention.' },
}
...
const catIndexToId = ['management','communication','commercial','developpement','securite']
...
export default function FormationsCatalogue({ onOpenModal }) {
  ...
  return (
    <section className="fcat-section" id="catalogue" aria-labelledby="fcat-title">
      <div className="container">
        ...
        <div className="fcat-header">
          ...
          <h2 id="fcat-title" className="fcat-header__title">
            {filtered.length} formation{filtered.length > 1 ? 's' : ''} <em>disponible{filtered.length > 1 ? 's' : ''}</em>
          </h2>
        ...
        <div className="fcat-filters-wrap" role="group" aria-label="Filtrer par catégorie">
          <div className="fcat-filters container">
            {CATS.map(cat => (
              ...
              >
                {cat.label}
                <span className="fcat-filter-badge" aria-label={`${counts[cat.id] || 0} formations`}>
                  {counts[cat.id] || 0}
                </span>
              </button>
            ))}
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map(formation => (
              <article
                ...
                onKeyDown={e => e.key === 'Enter' && onOpenModal(formation.key)}
                ...
                <div className="fcat-card__body">
                  ...
                  <div className="fcat-card__actions">
                    <button
                      ...
                      onClick={e => { e.stopPropagation(); onOpenModal(formation.key) }}
